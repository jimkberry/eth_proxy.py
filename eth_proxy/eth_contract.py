...
class EthContract(TransactionDelegate):
    ...
    # metadata (if any) or adds a "*" value that you will have to fix yourself 
    #
    # To use it, create at json file with at least 
    # { "code_filename": "etherpoker.sol" }
    ...
    def generate_metadata(self):
        ...
        if src_data == None:
            ...
            new_data = {}
            new_data['code_filename'] = self._data['code_filename']
            ...
            for meth in abi:
                ...
                mdata['gas'] = old_def.get('gas') or DEFAULT_META_GAS 
...
#      
    ...
    def tx_complete(self, delegate_data, tx_hash, contract_addr, gas_price,
        ...
        if delegate_data == self.CREATION_CONTEXT:
            ...
            if not self._addr:
                self.log.warning(err_msg or 'Contract creation failed. unknown reason')                
